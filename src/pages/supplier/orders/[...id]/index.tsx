...
function Order() {
  ...
  const { orderCustomer, orderSupplier } = useAppStore();
  ...
  return (
    <DashboardLayout isSupplier={true} title="Order" description={"Id: #" + id}>
      <div className="flex flex-col gap-4">
        <div className="flex justify-end items-center space-x-1">
          ...
          <FxDropdown
            align="right"
            trigger={
              <div className="flex items-center space-x-2 hover:bg-gray-50 px-4 py-2.5 border border-strokedark rounded-lg text-primary cursor-pointer">
                <CiMenuKebab size={20} />
              </div>
            }
            classnames={{
          ...
          >
            {dropdownItems.map((item) => (
              <DropdownItem
                ...
                className={
                  item.precedence?.includes(selectedOrder?.status ?? "")
            ...
            <Button
              ...
              onClick={() => {
                if (orderCustomer?.phone) {
                  let phoneNumber = orderCustomer?.phone.replace(/\D/g, "");
                  ...
                  window.open(
                    `https://wa.me/${phoneNumber}?text=Hello%20from%20Supplier%20-${orderSupplier?.branchName}%0ARegarding%20your%20order%20#${id}`,
                    "_blank"
                  );
                } else {
                  toast({
